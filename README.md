...
당근마켓에서 여러 동네(最大 495개)를 동시에 검색하고, 실시간으로 결과를 확인하며 최신순으로 정렬하는 서비스입니다.

## 주요 기능

- **다중 동네 동시 검색** — 원하는 만큼의 동네를 자유롭게 선택
- **실시간 스트리밍** — SSE로 동네별 결과가 도착하는 즉시 화면에 표시
- **최신순 정렬** — 등록일시 기준 내림차순, 새 글 程 앞에
- **검색 중지** — 언제든 검색 중지 가능 (버튼 或 `Esc` 키)
- **검색 기록** —以前的 검색어를 다시 사용可能
...
- 당근마켓 웹 스크래핑 방식 사용 — 网站 结构 变更 시 수정 필요
- 동네별 请求 사이에 0.1초 딜레이 (서버 보호)
- 검색 기록은 최대 100개까지 保存
